/**
 * Promart Peru products scraper. Uses the VTEX catalog system search API
...
import { normalizeVtexSearchResults } from "./normalize.ts";
...
export const promartProductsScraper = defineScraper({
  id: "promart-pe-products",
  store: "promart-pe",
  version: 1,
  match: [BASE + "/"],
  paramsSchema,
  ...
  selfCheck(): ProductInput[] {
    ...
    const json = JSON.parse(readFileSync(join(dir, "search-refrigeracion.json"), "utf8"));
    return normalizeVtexSearchResults(json).products;
  },
  async scrape(ctx): Promise<{ pagesProcessed: number; productsSeen: number }> {
    ...
    for (const url of urls) {
      ...
      const { products, ok } = normalizeVtexSearchResults(json);
      if (!ok) {
        ctx.logger.warn("promart search assertion failed", { url });
        continue;
      }
